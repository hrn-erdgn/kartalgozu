...
kredihacmidolar["Kredi Hacmi $"] = kredihacmidolar["TP_KREDI_L001"] / kredihacmidolar["TP_DK_USD_A_YTL"]
...
rezerv["Brut TCMB Dolar Rezerv"] = (rezerv["TP_AB_A02"] - rezerv["TP_AB_A10"]) / rezerv["TP_DK_USD_A_YTL"]
...
datafaiz.rename(columns={'TP_TRY_MT02':'32-91 gun vadeli', 'TP_TRY_MT03':'92-181 gun vadeli', 'TP_TRY_MT06':'Toplam A.O'},inplace=True)
...
kredifaizleri.rename(columns={'TP_KTF18':'Ticari Krediler', 'TP_KTFTUK01':'Tuketici Kredisi', 'TP_KTF101':'Ihtiyac Kredisi', 'TP_KTF11':'Tasir Kredisi', 'TP_KTF12':'Konut Kredisi'},inplace=True)
...
#son1ay = arz3['TP_PR_ARZ22-1'].tail(4).sum().sum()
#son6ay = arz3['TP_PR_ARZ22-1'].tail(24).sum().sum()
arz3["TP_PR_ARZ22-1"] = arz3["TP_PR_ARZ22-1"] * 52

arz3["3 aylik h.o"] = arz3["TP_PR_ARZ22-1"].rolling(window=13).mean()
#son1sene = arz3['TP_PR_ARZ22-1'].tail(48).sum().sum()
#print(son6ay)
#print(son1sene)
arz3.rename(columns={'TP_PR_ARZ22-1':'M3 Haftalik Degisim' },inplace=True)
GrafikCiz(arz3,2,0)
...
toplamkredidegisim.rename(columns={'TP_KREDI_L001-1':'Toplam Kredi Hacmi Degisim'},inplace=True)
...
tuketicikk.rename(columns={'TP_BFTUKKRE_L002-1':'Tuketici ve Kredi Kartlari'},inplace=True)
...
konut.rename(columns={'TP_BFTUKKRE_L005-1':'Konut Kredi Hacmi'},inplace=True)
GrafikCiz(konut,2,3)
...
arac.rename(columns={'TP_BFTUKKRE_L007-1':'Arac Kredi Hacmi'},inplace=True)
GrafikCiz(arac,2,3)

yabancidegisim = evds.get_data(["TP.MKNETHAR.M7", "TP.MKNETHAR.M8"], startdate=yilonce, enddate=bugun)
yabancidegisim.rename(columns={'TP_MKNETHAR_M7': 'Yabanci Hisse Senedi Alim', 'TP_MKNETHAR_M8': 'Yabanci DIBS Alim'}, inplace=True)
...
#plt.style.use('seaborn-pastel') # Temayi gosteriyor
